...
This section shows portal blades illustrating hosting plan behavior for a live Function App (Consumption Y1, Korea Central). PII is masked.

### Function App Overview

[Observed] The **Overview** blade shows the **App Service Plan** link as `KoreaCentralLinuxDynamicPlan (Y1: 0)`. The **Operating System** is Linux and the **Runtime version** is `4.1048.200.26180`. A migration banner warns that Linux Consumption will reach EOL on September 30, 2028:

![Function App Overview showing plan type and runtime details](../assets/operations/overview/01-function-app-overview.png)

[Inferred] The `(Y1: 0)` suffix confirms Consumption tier with 0 always-on instances. The dynamic plan name is auto-generated by Azure. For production decision-making, verify the plan tier from the App Service plan blade rather than relying on the plan name alone.

### App Service Plan Blade

[Observed] The **App Service plan** blade shows **Pricing plan: Y1**, **Instance Count: 0**, **Operating System: Linux**, **Location: Korea Central**, and **Zone redundant: Disabled**:

![App Service plan blade showing Y1 Consumption with 0 instances](../assets/operations/hosting/01-app-service-plan.png)

[Inferred] Instance Count 0 confirms scale-to-zero behavior, which is a defining characteristic of Consumption. Zone redundancy is not available on Consumption — consider Premium (EP) or Dedicated for zone-redundant deployments. The plan details directly map to the hosting plan comparison table below.

### Scale Out Blade

[Observed] The **Scale out** blade shows **Dynamic Scale out** with **Enforce Scale Out Limit: Yes** and **Maximum Scale Out Limit: 200**:

![Scale out blade showing Consumption plan maximum of 200 instances](../assets/operations/scaling/01-scale-out.png)

[Inferred] The 200-instance maximum is the platform ceiling for Linux Consumption. Windows Consumption has a lower default ceiling. Premium and Flex Consumption plans have different scale-out controls. Always set an explicit scale limit before production deployment.
