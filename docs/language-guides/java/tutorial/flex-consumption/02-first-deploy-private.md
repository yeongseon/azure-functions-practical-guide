...
You will provision a Flex Consumption Function App with Azure CLI, build and publish Java code from the Maven staging directory, and validate FC1 runtime behavior in Azure — all with private networking.
...
```bash
export BASE_NAME="jflex"
export RG="rg-func-java-flex-demo"
export APP_NAME="func-jflex-$(openssl rand -hex 4)"
export PLAN_NAME="flexdemo-plan"
export STORAGE_NAME="stjflex$(openssl rand -hex 4)"
export MI_NAME="jflex-identity"
export VNET_NAME="jflex-vnet"
export APPINSIGHTS_NAME="jflex-insights"
export LOCATION="koreacentral"
```

| Command/Parameter | Purpose |
|-------------------|---------|
| `export BASE_NAME="jflex"` | Sets the base name for resource naming conventions. |
| `export RG="rg-func-java-flex-demo"` | Defines the resource group name. |
| `export APP_NAME="func-jflex-..."` | Sets the name for the Azure Function App (globally unique). |
| `export PLAN_NAME="flexdemo-plan"` | Defines the Flex Consumption hosting plan name. |
| `export STORAGE_NAME="stjflex..."` | Specifies the storage account name for the function (globally unique). |
| `export MI_NAME="jflex-identity"` | Sets the name for the user-assigned managed identity. |
| `export VNET_NAME="jflex-vnet"` | Defines the Virtual Network name. |
| `export APPINSIGHTS_NAME="jflex-insights"` | Sets the Application Insights resource name. |
...
```json
{
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-java-flex-demo",
  "location": "koreacentral",
  "name": "rg-func-java-flex-demo",
  "properties": {
...
```json
{
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-java-flex-demo/providers/Microsoft.Storage/storageAccounts/<storage-name>",
  "kind": "StorageV2",
  "location": "koreacentral",
  "name": "<storage-name>",
  "properties": {
...
{
  "clientId": "<object-id>",
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-java-flex-demo/providers/Microsoft.ManagedIdentity/userAssignedIdentities/jflex-identity",
  "location": "koreacentral",
  "name": "jflex-identity",
  "principalId": "<object-id>",
  "resourceGroup": "rg-func-java-flex-demo",
  "tenantId": "<tenant-id>"
}
```
...
```json
{
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-java-flex-demo/providers/Microsoft.Authorization/roleAssignments/<object-id>",
  "principalId": "<object-id>",
  "principalType": "ServicePrincipal",
  "roleDefinitionName": "Storage Blob Data Owner",
  "scope": "/subscriptions/<subscription-id>/resourceGroups/rg-func-java-flex-demo/providers/Microsoft.Storage/storageAccounts/<storage-name>"
}
```
...
{
  "newVNet": {
    "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-java-flex-demo/providers/Microsoft.Network/virtualNetworks/jflex-vnet",
    "location": "koreacentral",
    "name": "jflex-vnet",
    "provisioningState": "Succeeded"
  }
...
```json
{
  ...
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-java-flex-demo/providers/Microsoft.Network/virtualNetworks/jflex-vnet/subnets/subnet-private-endpoints",
  "name": "subnet-private-endpoints",
  "provisioningState": "Succeeded"
}
```
...
```json
{
  ...
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-java-flex-demo/providers/microsoft.insights/components/jflex-insights",
  "name": "jflex-insights"
}
```
...
| `--runtime java` | Sets the application language runtime to Java. |
| `--runtime-version 17` | Selects the specific Java version. |
...
```json
{
  ...
  "httpsOnly": false,
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-java-flex-demo/providers/Microsoft.Web/sites/<app-name>",
  "identity": {
    "type": "UserAssigned"
  },
...
!!! note "Auto-created Application Insights"
    `az functionapp create` automatically creates its own Application Insights instance named after the function app. This is separate from the `jflex-insights` instance created in Step 11. The auto-created instance can be deleted from the portal if you prefer to use only the manually created one.
...
```json
{
  "storage": {
    "authentication": {
      ...
      "userAssignedIdentityResourceId": "/subscriptions/<subscription-id>/resourcegroups/rg-func-java-flex-demo/providers/Microsoft.ManagedIdentity/userAssignedIdentities/jflex-identity"
    },
    "type": "blobcontainer",
...
!!! tip "Placeholder settings for reference app triggers"
    The reference app in `apps/java/` includes EventHub, Queue, and Timer triggers. These require additional app settings to prevent host startup errors. Add them after the core settings:
...
```json
{
  ...
  "name": "jflex-vnet",
  "resourceGroup": "rg-func-java-flex-demo",
  "subnetResourceId": "/subscriptions/<subscription-id>/resourceGroups/rg-func-java-flex-demo/providers/Microsoft.Network/virtualNetworks/jflex-vnet/subnets/subnet-integration"
}
```

### Step 16: Build and Publish Code with Core Tools

Flex does not expose Kudu/SCM workflows; publish with Core Tools (or One Deploy in CI/CD). Java requires a Maven build step before publishing.
...
| Command/Parameter | Purpose |
|-------------------|---------|
| `cd apps/java` | Navigates to the Java function application source directory. |
| `mvn clean package` | Compiles the Java source and packages it into the Maven staging directory. |
| `cd target/azure-functions/azure-functions-java-guide` | Navigates to the Maven staging directory containing the deployment artifacts. |
...
- `GET /api/info` → `{"name": "azure-functions-java-guide", "version": "1.0.0", "java": "17", "environment": "development"}`
