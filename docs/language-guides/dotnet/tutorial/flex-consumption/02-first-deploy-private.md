...
# 02 - First Deploy with Private Networking (Flex Consumption)
...
You will provision a Flex Consumption Function App with Azure CLI, publish .NET 8 isolated worker code, and validate FC1 runtime behavior in Azure — all with private networking.
...
```bash
export BASE_NAME="dnetflex"
export RG="rg-func-dotnet-flex-demo"
export APP_NAME="func-dnetflex-$(openssl rand -hex 4)"
export PLAN_NAME="flexdemo-plan"
export STORAGE_NAME="stdnetflex$(openssl rand -hex 4)"
export MI_NAME="dnetflex-identity"
export VNET_NAME="dnetflex-vnet"
export APPINSIGHTS_NAME="dnetflex-insights"
export LOCATION="koreacentral"
```

| Command/Parameter | Purpose |
|-------------------|---------|
| `export BASE_NAME="dnetflex"` | Sets the base name for resource naming conventions. |
| `export RG="rg-func-dotnet-flex-demo"` | Defines the resource group name. |
| `export APP_NAME="func-dnetflex-..."` | Sets the name for the Azure Function App (globally unique). |
| `export PLAN_NAME="flexdemo-plan"` | Defines the Flex Consumption hosting plan name. |
| `export STORAGE_NAME="stdnetflex..."` | Specifies the storage account name for the function (globally unique). |
| `export MI_NAME="dnetflex-identity"` | Sets the name for the user-assigned managed identity. |
| `export VNET_NAME="dnetflex-vnet"` | Defines the Virtual Network name. |
| `export APPINSIGHTS_NAME="dnetflex-insights"` | Sets the Application Insights resource name. |
...
```json
{
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-dotnet-flex-demo",
  "location": "koreacentral",
  "name": "rg-func-dotnet-flex-demo",
  "properties": {
...
```json
{
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-dotnet-flex-demo/providers/Microsoft.Storage/storageAccounts/<storage-name>",
  "kind": "StorageV2",
  "location": "koreacentral",
  "name": "<storage-name>",
  "properties": {
...
{
  "clientId": "<object-id>",
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-dotnet-flex-demo/providers/Microsoft.ManagedIdentity/userAssignedIdentities/dnetflex-identity",
  "location": "koreacentral",
  "name": "dnetflex-identity",
  "principalId": "<object-id>",
  "resourceGroup": "rg-func-dotnet-flex-demo",
  "tenantId": "<tenant-id>"
}
```
...
```json
{
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-dotnet-flex-demo/providers/Microsoft.Authorization/roleAssignments/<object-id>",
  "principalId": "<object-id>",
  "principalType": "ServicePrincipal",
  "roleDefinitionName": "Storage Blob Data Owner",
  "scope": "/subscriptions/<subscription-id>/resourceGroups/rg-func-dotnet-flex-demo/providers/Microsoft.Storage/storageAccounts/<storage-name>"
}
```
...
{
  "newVNet": {
    "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-dotnet-flex-demo/providers/Microsoft.Network/virtualNetworks/dnetflex-vnet",
    "location": "koreacentral",
    "name": "dnetflex-vnet",
    "provisioningState": "Succeeded"
  }
...
```json
{
  ...
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-dotnet-flex-demo/providers/Microsoft.Network/virtualNetworks/dnetflex-vnet/subnets/subnet-private-endpoints",
  "name": "subnet-private-endpoints",
  "provisioningState": "Succeeded"
}
```
...
```json
{
  ...
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-dotnet-flex-demo/providers/microsoft.insights/components/dnetflex-insights",
  "name": "dnetflex-insights"
}
```
...
| `--runtime dotnet-isolated` | Sets the application runtime to .NET 8 isolated worker. |
| `--runtime-version 8.0` | Selects the specific .NET version. |
...
```json
{
  ...
  "httpsOnly": false,
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-dotnet-flex-demo/providers/Microsoft.Web/sites/<app-name>",
  "identity": {
    "type": "UserAssigned"
  },
...
!!! note "Auto-created Application Insights"
    `az functionapp create` automatically creates its own Application Insights instance named after the function app. This is separate from the `dnetflex-insights` instance created in Step 11. The auto-created instance can be deleted from the portal if you prefer to use only the manually created one.
...
```json
{
  "storage": {
    "authentication": {
      ...
      "userAssignedIdentityResourceId": "/subscriptions/<subscription-id>/resourcegroups/rg-func-dotnet-flex-demo/providers/Microsoft.ManagedIdentity/userAssignedIdentities/dnetflex-identity"
    },
    "type": "blobcontainer",
...
!!! tip "Placeholder settings for reference app triggers"
    The reference app in `apps/dotnet/` includes EventHub, Queue, and Timer triggers. These require additional app settings to prevent host startup errors. Add them after the core settings:
...
```json
{
  ...
  "name": "dnetflex-vnet",
  "resourceGroup": "rg-func-dotnet-flex-demo",
  "subnetResourceId": "/subscriptions/<subscription-id>/resourceGroups/rg-func-dotnet-flex-demo/providers/Microsoft.Network/virtualNetworks/dnetflex-vnet/subnets/subnet-integration"
}
```

### Step 16: Build and Publish Code with Core Tools
...
| Command/Parameter | Purpose |
|-------------------|---------|
| `cd apps/dotnet` | Navigates to the .NET function application source directory. |
| `dotnet publish --configuration Release --output ./publish` | Compiles the project and outputs build artifacts to the publish directory. |
| `cd publish` | Navigates to the compiled output directory. |
| `func azure functionapp publish "$APP_NAME" --dotnet-isolated` | Packages and uploads the compiled project to the Azure Function App. |
...
Functions in <app-name>:
    ...
    logLevels - [httpTrigger]
    externalDependency - [httpTrigger]
    testError - [httpTrigger]
...
- `GET /api/info` → `{"name": "azure-functions-dotnet-guide", "version": "1.0.0", "dotnet": ".NET 8.0.23", "os": "Linux", "environment": "production"}`
