...
2. Create resource group and storage account.
    ...
    export STORAGE_NAME="stndprem$(openssl rand -hex 4)"
    export APP_NAME="func-ndprem-$(openssl rand -hex 4)"
    export PLAN_NAME="plan-ndprem-$(openssl rand -hex 4)"
...
3. Create the Premium plan and Function App (Linux example).
    ...
    | `--runtime "node"` | Sets the language runtime to Node.js. |
    | `--runtime-version "20"` | Specifies the Node.js version. |
...
4. Configure app settings using classic `siteConfig.appSettings` model values.
    ...
    | `"FUNCTIONS_WORKER_RUNTIME=node"` | Sets the runtime worker to Node.js. |
...
9. Publish function code (Premium supports file share-based deployment and SCM/Kudu).
    ...
    | Command/Parameter | Purpose |
    |-------------------|---------|
    | `cd apps/nodejs` | Navigates to the Node.js project directory. |
...
```json
{
  "id": "/subscriptions/<subscription-id>/resourceGroups/rg-func-node-prem-demo/providers/Microsoft.Web/sites/<app-name>",
  "location": "koreacentral",
  "name": "<app-name>",
  "state": "Running",
