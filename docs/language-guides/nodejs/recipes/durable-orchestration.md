...
Durable orchestration entry points, activities, and fan-out/fan-in control.
...
app.http('startOrchestration', { methods: ['POST'], handler: async (_request, context) => { context.log('Start durable orchestration request'); return { status: 202, jsonBody: { orchestration: 'accepted' } }; } });
```
