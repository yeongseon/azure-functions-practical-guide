...
Assign identity and access downstream Azure resources without secrets.
...
app.http('identityCheck', { methods: ['GET'], handler: async (_request, context) => { context.log('Managed identity path'); return { body: 'identity-enabled' }; } });
```
