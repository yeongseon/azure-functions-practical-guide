...
HTTPS custom domain and certificate lifecycle for production HTTP endpoints.
...
app.http('healthz', { methods: ['GET'], route: 'healthz', handler: async (_request, context) => { context.log('healthz'); return { body: 'ok' }; } });
```
