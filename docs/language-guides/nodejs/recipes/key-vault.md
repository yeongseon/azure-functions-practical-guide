...
Using `@azure/identity` and `@azure/keyvault-secrets` from Node.js functions.
...
app.http('kvSecret', { methods: ['GET'], handler: async (_request, context) => { context.log('Retrieve secret via managed identity'); return { status: 200, jsonBody: { secretName: 'ApiKey' } }; } });
```
