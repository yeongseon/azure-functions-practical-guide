...
Patterns for reading and writing Cosmos DB documents with managed identity.
...
app.http('cosmosRead', { methods: ['GET'], handler: async (_request, context) => { context.log('Read from Cosmos DB'); return { status: 200, jsonBody: { source: 'cosmos', ok: true } }; } });
```
